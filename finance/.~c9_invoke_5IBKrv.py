...
from helpers import apology, login_required, lookup, usd, index_stock
...
def index():
    ...
    portfolio = db.execute("SELECT stock.ticker, stock.name, portfolio.quantity FROM stock JOIN users, portfolio ON users.id = portfolio.user_id AND stock.id = portfolio.stock_id WHERE users.id IN (SELECT id FROM users WHERE id = ?)", session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        if session["cash"] - purchase_price < 0:
            return apology("insufficient funds to complete purchase", 403)


        # Record purchase
        db.execute("INSERT INTO account_history (user_id, stock_id, transaction_type, quantity, price) VALUES (?, ?, 'BUY', ?, ?)", session["user_id"], stock_id, request.form.get("shares"), stock_info["price"])


        # Update user's portfolio
        if len(db.execute("SELECT * FROM portfolio WHERE user_id = (?) AND stock_id = (?)", session["user_id"], stock_id)) != 0:
            db.execute("UPDATE portfolio SET quantity = quantity + (?) WHERE user_id = (?) AND stock_id = (?)",  request.form.get("shares"), session["user_id"], stock_id)
        else:
            db.execute("INSERT INTO portfolio (user_id, stock_id, quantity) VALUES (?, ?, ?)", session["user_id"], stock_id, request.form.get("shares"))
...
def history():
    ...
    # Get ticker, quantity,
    history = db.execute("SELECT stock.ticker, quantity, price, transaction_type, date FROM account_history JOIN stock ON stock.id = account_history.stock_id WHERE user_id = (?) ORDER BY date DESC", session["user_id"])
...
def register():
    ...
    if request.method == "POST":
        ...
        elif request.form.get("password") != request.form.get("password_reentry"):
...
def sell():
    ...
    if request.method == "POST":
        # Ensure an order of positive int has been placed
        user_shares = db.execute("SELECT quantity FROM portfolio WHERE user_id = (?) AND stock_id IN (SELECT id FROM stock WHERE ticker = (?))", session["user_id"], request.form.get('symbol'))
        ...
        db.execute("UPDATE users SET cash = (?) WHERE id = (?)", session["cash"], session["user_id"])
        db.execute("UPDATE portfolio SET quantity = quantity - (?) WHERE user_id = (?) AND stock_id = (?)", request.form.get("shares"), session["user_id"], stock_id[0]["id"])
        db.execute("DELETE FROM portfolio WHERE quantity = 0 AND user_id = (?)", session["user_id"])

        # Update transaction log
        db.execute("INSERT INTO account_history (user_id, stock_id, transaction_type, quantity, price) VALUES (?, ?, 'SELL', ?, ?)", session["user_id"], stock_id[0]["id"], request.form.get("shares"), price["price"])

        flash("Sell order successful", "info")
        return redirect("/")
    else:
        portfolio = db.execute("SELECT stock.ticker FROM stock JOIN users, portfolio ON users.id = portfolio.user_id AND stock.id = portfolio.stock_id WHERE users.id IN (SELECT id FROM users WHERE id = ?)", session["user_id"])
